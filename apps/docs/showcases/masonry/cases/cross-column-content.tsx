...
const createExample = (locale: 'en' | 'zh') => {
  ...
  const MasonrySpanDemo = () => {
    return (
      <Masonry
        aria-label={copy('包含跨列内容的 Masonry 布局示例')}
        ...
        items={[
          ...createPreviewItems(masonryItems.slice(0, 3)),
          {
            ...
            content: (
              <>
                <span>FULL</span>
                <strong>{copy('发布检查')}</strong>
                <p>{copy('等待前面所有列结束，再独占整行展示发布结论。')}</p>
              </>
            ),
            key: 'release-check',
            role: 'listitem',
  ...
  return MasonrySpanDemo;
};

const ZhExample = createExample('zh');
const EnExample = createExample('en');

export default function MasonryCase03({
  locale = 'zh',
}: {
  locale?: 'en' | 'zh';
}) {
