...
const createExample = (locale: 'en' | 'zh') => {
    ...
    return (
      <div className="display-table-managed">
        <Table
          ...
          data={releaseRecords}
          footer={(rows) => copy(`当前页 ${rows.length} 条发布记录`)}
          rowKey="version"
          search={{
            columnKeys: ['version'],
            placeholder: copy('筛选版本…'),
          }}
          pagination={{
            ariaLabels: {
              more: copy('更多页面'),
              ...
              summary === 'hidden'
                ...
                : summary === 'custom'
                  ? (total, current, pageCount) =>
                      copy(`发布 ${current}/${pageCount} · 共 ${total} 条记录`)
                  : true,
          }}
  ...
  return TableManagedDemo;
};

const ZhExample = createExample('zh');
const EnExample = createExample('en');

export default function TableCase01({
  locale = 'zh',
}: {
  locale?: 'en' | 'zh';
}) {
