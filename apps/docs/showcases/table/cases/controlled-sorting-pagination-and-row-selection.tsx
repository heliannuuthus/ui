...
const createExample = (locale: 'en' | 'zh') => {
  ...
  const TableControlledStateDemo = () => {
    ...
    return (
      <div className="display-table-managed">
        ...
        <Table
          ...
          rowKey="version"
          rowSelection={{
            disabled: (row) => row.status === copy('运行中'),
            labels: {
              all: (rows) => copy(`选择当前页 ${rows.length} 条发布记录`),
              item: (row) => copy(`选择 ${row.version}`),
            },
  ...
  return TableControlledStateDemo;
};

const ZhExample = createExample('zh');
const EnExample = createExample('en');

export default function TableCase05({
  locale = 'zh',
}: {
  locale?: 'en' | 'zh';
}) {
