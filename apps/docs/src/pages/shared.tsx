...
const packageManagerIcons: Record<
  PackageManager,
  { color: string; path: string; title: string }
> = {
  pnpm: {
    color: '#f69220',
    title: 'pnpm',
...
const PackageManagerLabel = ({ manager }: { manager: PackageManager }) => {
  const icon = packageManagerIcons[manager];

  return (
    <span className="install-tab-label">
      <svg aria-hidden="true" fill={icon.color} viewBox="0 0 24 24">
        <path d={icon.path} />
      </svg>
...
export const InstallTabs = ({
  ...
  <Tabs
    ...
    items={Object.entries(installCommands).map(([manager, command]) => ({
      value: manager,
      label: <PackageManagerLabel manager={manager as PackageManager} />,
      content: (
        <CodePanel
          code={command}
